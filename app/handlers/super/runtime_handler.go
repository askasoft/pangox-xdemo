...
func runtimeMemStats() any {
	...
	msm.Set("Mallocs", num.HumanSize(ms.Mallocs))
	msm.Set("Frees", num.Comma(ms.Frees))
	msm.Set("Alloc", num.HumanSize(ms.Alloc))
	msm.Set("HeapAlloc", num.HumanSize(ms.HeapAlloc))
	msm.Set("TotalAlloc", num.HumanSize(ms.TotalAlloc))
	msm.Set("Sys", num.HumanSize(ms.Sys))
	msm.Set("Lookups", num.Comma(ms.Lookups))
	msm.Set("HeapAlloc", num.HumanSize(ms.HeapAlloc))
	msm.Set("HeapIdle", num.HumanSize(ms.HeapIdle))
	msm.Set("HeapInuse", num.HumanSize(ms.HeapInuse))
	msm.Set("HeapReleased", num.HumanSize(ms.HeapReleased))
	msm.Set("HeapObjects", num.Comma(ms.HeapObjects))
	msm.Set("StackInuse", num.HumanSize(ms.StackInuse))
	msm.Set("StackSys", num.HumanSize(ms.StackSys))
	msm.Set("MSpanInuse", num.HumanSize(ms.MSpanInuse))
	msm.Set("MSpanSys", num.HumanSize(ms.MSpanSys))
	msm.Set("MCacheInuse", num.HumanSize(ms.MCacheInuse))
	msm.Set("MCacheSys", num.HumanSize(ms.MCacheSys))
